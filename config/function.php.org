...
function kd_trans_auto()
	{
		$sql = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT MAX(RIGHT(kd_transaksi,5)) AS notrans
		...
		if($m['notrans'] <> NULL)
		{
			...
			if(strlen($kd) == 1)
			{
				$no = "TRS".date('dmy')."0000".$kd;
			}
			elseif (strlen($kd) == 2) {
				$no = "TRS".date('dmy')."000".$kd;
			}
			elseif (strlen($kd) == 3) {
				$no = "TRS".date('dmy')."00".$kd;
			}
			elseif (strlen($kd) == 4) {
				$no = "TRS".date('dmy')."0".$kd;
			}
			else {
				$no = "TRS".date('dmy').$kd;
			}
	...
	{
		$sql = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT MAX(RIGHT(kd_transmasuk,5)) AS notrans
	...
	{
		$sql = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT MAX(RIGHT(kd_transkeluar,5)) AS notrans
...
function query($qry) {
	$result = mysqli_query($GLOBALS["___mysqli_ston"], $qry) or die("Gagal melakukan query pada :
	 <br>$qry<br><br>Kode Salah : <br>&nbsp;&nbsp;&nbsp;" . mysqli_error($GLOBALS["___mysqli_ston"]) . "!");
	return $result;
}
